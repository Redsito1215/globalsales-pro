...
SCOPE_META: dict[str, dict[str, str]] = {
    SCOPE_SIMPLE: {
        "label": "Informes simples",
        "subtitle": "Operativos del día a día (pedidos, stock, soporte, proveedores).",
        "prefix": "RS",
        "data_layer": "operativo",
    },
    SCOPE_COMPOUND: {
        "label": "Informes compuestos",
        "subtitle": "Análisis estratégico con agregaciones y modelo de datos (ventas, márgenes, rankings).",
        "prefix": "RC",
...
def _local_reason(*, rep: dict[str, Any], prompt: str, score: float) -> str:
    ...
    if not (prompt or "").strip():
        return f"{tipo} recomendado para la operación diaria de GLOBTRADE." if tipo else "Informe frecuente en el catálogo."
    if score >= 10:
...
def engine_status() -> dict[str, Any]:
    ...
    return {
        ...
        "engine_default": "openai" if configured else "local",
        "engine_note": (
            "OpenAI activo: recomendaciones y textos más contextuales."
            if configured
            else "Motor local en español (sin API key): interpreta sinónimos y frases del catálogo RS/RC."
        ),
    }
...
def _call_llm(messages: list[dict[str, str]], *, temperature: float = 0.2) -> str | None:
...
def _llm_recommendations(*, prompt: str, limit: int = 5, scope: str = SCOPE_ALL) -> list[dict[str, Any]] | None:
    ...
    system = (
        "Eres analista de negocio en GLOBTRADE. Recomienda informes del catálogo dado."
        + scope_note
...
def recommend_reports(*, prompt: str | None = None, limit: int = 5, scope: str = SCOPE_ALL) -> dict[str, Any]:
    ...
    engine = "local"
    recs = _local_recommendations(prompt=text, limit=limit, scope=scope)
    if _ai_configured():
        llm_recs = _llm_recommendations(prompt=text, limit=limit, scope=scope)
        if llm_recs:
            recs = llm_recs
            engine = "openai"
...
def _llm_pick_report(prompt: str, *, scope: str = SCOPE_ALL) -> dict[str, Any] | None:
    ...
    system = (
        "Elige el informe del catálogo GLOBTRADE que mejor responde la petición del usuario."
        + scope_note
        + " Responde SOLO JSON: "
...
def generate_report_from_prompt(
    *,
    ...
    if not pick:
        local = _local_recommendations(prompt=text, limit=1, scope=scope)
        if not local:
            raise ValueError("no_match")
        best = local[0]
        pick = {
            "report_id": best["report_id"],
            "custom_name": _custom_title(text, best["name"]),
            "search_query": _extract_search_query(text),
            "reason": best.get("reason") or "Coincidencia local con el catálogo.",
            "narrative": None,
        }
