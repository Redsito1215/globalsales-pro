"""Paso 4 — TRANSFORMAR en MongoDB: hechos, dimensiones y tablas SQL (regions, orders, ...)."""
...
def main(mongo_uri: str | None = None, mongo_db: str | None = None) -> None:
    ...
    rmap = {}
    regions = []
    for i, name in enumerate(sorted(df["region"].dropna().unique()), start=1):
        regions.append({"region_id": i, "name": name, "description": f"Zona comercial: {name}"})
        rmap[name] = i
    ...
    cmap = {}
    paises = []
    for i, row in enumerate(df[["country", "region"]].drop_duplicates().sort_values("country").itertuples(index=False), start=1):
    ...
    catmap = {}
    categorias = []
    for i, name in enumerate(sorted(df["item_type"].dropna().unique()), start=1):
        categorias.append({"category_id": i, "name": name, "description": CATEGORY_DESC.get(name, "Categoría general")})
        catmap[name] = i
    ...
    productos, pid = [], 1
    part = df.groupby(["item_type", "unit_price", "unit_cost"], as_index=False).first()
    for item_type, g in part.groupby("item_type"):
        for n, row in enumerate(g.itertuples(), start=1):
            up, uc = float(row.unit_price), float(row.unit_cost)
            productos.append({
                "product_id": pid, "name": f"{item_type} - Ref.{n}",
                "category_id": catmap[item_type], "unit_price": up, "unit_cost": uc,
                "margin_pct": round(((up - uc) / up * 100) if up else 0, 2),
            })
            pid += 1
    db["dim_producto"].insert_many(productos)

    db["dim_cliente"].delete_many({})
    client_lookup, clientes = {}, []
    for i, row in enumerate(df[["country", "sales_channel"]].drop_duplicates().itertuples(index=False), start=1):
        ...
        clientes.append({
            "client_id": i, "name": f"Cliente-{i:05d}",
            "country_id": cmap[row.country], "channel_id": chmap.get(row.sales_channel, 1),
            "email": None, "phone": None, "created_at": datetime.now().date().isoformat(),
        })
    ...
    for i, fecha in enumerate(sorted(df["order_date"].dropna().dt.date.unique()), start=1):
        tiempos.append({"tiempo_id": i, "fecha_id": fecha.isoformat(), "anio": fecha.year, "mes": fecha.month, "trimestre": (fecha.month - 1) // 3 + 1})
        tmap[fecha.isoformat()] = i
    db["dim_tiempo"].insert_many(tiempos)

    db["fact_ventas"].delete_many({})
    hechos, order_lines, orders_map = [], [], {}
    for i, row in enumerate(df.itertuples(), start=1):
        ...
        hechos.append({
            "venta_id": i, "order_id": oid, "tiempo_id": tmap.get(fecha), "fecha_id": fecha,
            "region_id": rmap.get(row.region), "country_id": cmap.get(row.country),
            "category_id": catmap.get(row.item_type), "channel_id": chmap.get(row.sales_channel),
            "priority_id": pmap.get(row.order_priority),
            "client_id": client_lookup.get((row.country, row.sales_channel)),
            "units_sold": u, "unit_price": up, "unit_cost": uc,
            "total_revenue": float(row.total_revenue), "total_cost": float(row.total_cost),
            "total_profit": float(row.total_profit),
            "line_revenue": round(u * up, 2), "line_cost": round(u * uc, 2), "line_profit": round(u * (up - uc), 2),
        })
        order_lines.append({
            "line_id": i, "order_id": int(oid) if oid.isdigit() else oid,
        ...
        if oid not in orders_map:
            ...
            orders_map[oid] = {
                ...
                "client_id": client_lookup.get((row.country, row.sales_channel)),
                "country_id": cmap.get(row.country), "channel_id": chmap.get(row.sales_channel),
                "priority_id": pmap.get(row.order_priority),
                "order_date": od.isoformat() if od else None, "ship_date": sd.isoformat() if sd else None,
                "delivery_days": (sd - od).days if od and sd else None,
                "total_revenue": float(row.total_revenue), "total_cost": float(row.total_cost),
                "total_profit": float(row.total_profit), "status": "Delivered",
            }
    ...
    for kid, (keys, g) in enumerate(df.groupby(["year", "month", "region", "item_type"], dropna=False), start=1):
        ...
        kpis.append({
            "kpi_id": kid, "year": int(keys[0]) if pd.notna(keys[0]) else None,
            "month": int(keys[1]) if pd.notna(keys[1]) else None,
            "region": keys[2], "item_type": keys[3], "total_orders": len(g),
            "total_units": int(g["units_sold"].sum()), "total_revenue": round(float(rev), 2),
            "total_cost": round(float(g["total_cost"].sum()), 2), "total_profit": round(float(prof), 2),
    ...
    print("Espejo nombres SQL (schema.sql):")
    _mirror(db, "dim_region", "regions")
    _mirror(db, "dim_pais", "countries")
    _mirror(db, "dim_categoria", "product_categories")
    _mirror(db, "dim_producto", "products")
